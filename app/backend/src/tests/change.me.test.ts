...
import allTeams = require("./mocks/allTeams.json");
import allMatchesHome = require("./mocks/allMatchesHome.json");
...
describe("Tests leaderboard endpoints", () => {
  before(() => {
    sinon.stub(Teams, "findAll").resolves(allTeams as any);
    sinon.stub(Matches, "findAll").resolves(allMatchesHome as any);
  });
...
describe("/matches endpoint", () => {
  before(async () => {
    ...
    sinon.stub(Camelizer, "snakeToCamel").returns(allMatchesCamelized as any);
  });
